...
export default async function handler(request: NextApiRequest, response: NextApiResponse) {
  ...
  const delivery = await sendTestNewsletter(body.subject, body.body, body.email);
  await recordActivity({ actor: principal.email, action: "newsletter_test_sent", entityType: "newsletter", metadata: { recipient: body.email } });
